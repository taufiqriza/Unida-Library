        ...
        <div class="px-4 py-3 border-b" :class="$store.darkMode ? 'border-slate-700' : 'border-slate-100'">
            <div class="flex items-center justify-between">
                <h3 class="font-semibold text-sm" :class="$store.darkMode ? 'text-slate-100' : 'text-slate-900'">
                    <i class="fas fa-circle text-emerald-500 text-[8px] mr-2"></i>Staff Online
                </h3>
        ...
        <div class="max-h-64 overflow-y-auto">
            ...
            <div class="px-4 py-2.5 flex items-center gap-3 border-b last:border-0" :class="$store.darkMode ? 'border-slate-700 hover:bg-slate-700/50' : 'border-slate-50 hover:bg-slate-50'">
                <div class="relative">
                    ...
                    <span class="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 {{ $staff->is_online ? 'bg-emerald-500' : 'bg-amber-500' }}" :class="$store.darkMode ? 'border-slate-800' : 'border-white'"></span>
                </div>
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px]" :class="$store.darkMode ? 'text-slate-500' : 'text-slate-400'">
                            {{ $staff->is_online ? 'Online' : $staff->last_seen_at?->diffForHumans() }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="px-4 py-8 text-center">
                ...
                <p class="text-sm" :class="$store.darkMode ? 'text-slate-500' : 'text-slate-400'">Tidak ada staff online</p>
            </div>
            @endforelse
        </div>
    </div>
