...
@if($activeTab === 'online' && !$isLoading && count($pageViews) > 0)
@script
<script>
const gaData = $wire.pageViews;
let gaChart = null;

function initGaChart() {
    const el = document.getElementById('gaTrafficChart');
    if (!el || !gaData || gaData.length === 0) return;
    ...
    gaChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: gaData.map(d => d.date),
            datasets: [{
                label: 'Pageviews',
                data: gaData.map(d => d.views),
