<x-opac.layout :title="__('opac.pages.virtual_tour.title')">
    <x-opac.page-header 
        :title="__('opac.pages.virtual_tour.title')" 
        :subtitle="__('opac.pages.virtual_tour.subtitle')"
        :breadcrumbs="[['label' => __('opac.pages.virtual_tour.breadcrumb')], ['label' => __('opac.pages.virtual_tour.title')]]"
    />
    ...
    <section class="max-w-4xl mx-auto px-4 py-6 lg:py-10">
        ...
        <div class="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-2xl p-5 lg:p-6 border border-cyan-100 mb-6">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-900 mb-1">{{ __('opac.pages.virtual_tour.immersive_title') }}</h3>
                    <p class="text-gray-600 text-sm">{{ __('opac.pages.virtual_tour.immersive_desc') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-primary-600 to-primary-700 px-4 py-3 flex items-center justify-between">
                <h3 class="text-white font-semibold text-sm flex items-center gap-2">
                    <i class="fas fa-street-view"></i> {{ __('opac.pages.virtual_tour.tour_title') }}
                </h3>
                <a href="https://tour.digilib-unida.id" target="_blank" class="text-xs text-primary-200 hover:text-white transition">
                    <i class="fas fa-external-link-alt mr-1"></i> {{ __('opac.pages.virtual_tour.open_new_tab') }}
                </a>
            </div>
        ...
        <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-6">
            <h3 class="font-bold text-gray-900 mb-3 flex items-center gap-2">
                <i class="fas fa-link text-primary-500"></i> {{ __('opac.pages.virtual_tour.direct_access') }}
            </h3>
            <a href="https://tour.digilib-unida.id" target="_blank" class="flex items-center justify-between p-4 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl hover:from-cyan-100 hover:to-blue-100 transition group">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">{{ __('opac.pages.virtual_tour.click_to_open') }}</p>
                    </div>
                </div>
        ...
        <!-- Features -->
        <h3 class="font-bold text-gray-900 mb-4 text-base lg:text-lg">{{ __('opac.pages.virtual_tour.explore_title') }}</h3>
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="text-center p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <i class="fas fa-door-open text-blue-600"></i>
                </div>
                <p class="text-xs font-medium text-gray-900">{{ __('opac.pages.virtual_tour.lobby') }}</p>
            </div>
            <div class="text-center p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <p class="text-xs font-medium text-gray-900">{{ __('opac.pages.virtual_tour.reading_room') }}</p>
            </div>
            <div class="text-center p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <i class="fas fa-books text-purple-600"></i>
                </div>
                <p class="text-xs font-medium text-gray-900">{{ __('opac.pages.virtual_tour.collection_shelves') }}</p>
            </div>
            <div class="text-center p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center mx-auto mb-2">
                    <i class="fas fa-users text-orange-600"></i>
                </div>
                <p class="text-xs font-medium text-gray-900">{{ __('opac.pages.virtual_tour.discussion_room') }}</p>
            </div>
        </div>
    </section>
</x-opac.layout>
